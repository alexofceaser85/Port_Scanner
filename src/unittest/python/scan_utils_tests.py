...
class TestHandleScanConnection(unittest.TestCase):

    def test_should_not_scan_ports_of_type_none(self):

        PARENT_CONNECTION, CHILD_CONNECTION = multiprocessing.Pipe()
        PROCESSES = multiprocessing.Process(target=scan_utils.handle_scan_connection, args=(None, None, 3281, CHILD_CONNECTION, ))
        PROCESSES.start()
        PROCESSES.join()
    ...
    def test_scan_one_closed_port(self):
        ...
        PARENT_CONNECTION, CHILD_CONNECTION = multiprocessing.Pipe()
        PROCESSES = multiprocessing.Process(target=scan_utils.handle_scan_connection, args=('127.0.0.1', 3280, 3281, CHILD_CONNECTION, ))
        PROCESSES.start()
        PROCESSES.join()
        OUTPUT = PARENT_CONNECTION.recv()
        ...
        self.assertEqual(test_dictionary, OUTPUT)

    def test_scan_one_open_port(self):
    ...
    def test_scan_many_closed_ports(self):

        PARENT_CONNECTION, CHILD_CONNECTION = multiprocessing.Pipe()
        PROCESSES = multiprocessing.Process(target=scan_utils.handle_scan_connection, args=('127.0.0.1', 3280, 3290, CHILD_CONNECTION, ))
        PROCESSES.start()
        PROCESSES.join()
        OUTPUT = PARENT_CONNECTION.recv()
    ...
    def test_scan_many_open_ports(self):
        ...
        PARENT_CONNECTION, CHILD_CONNECTION = multiprocessing.Pipe()
        PROCESSES = multiprocessing.Process(target=scan_utils.handle_scan_connection, args=('127.0.0.1', 3280, 3285, CHILD_CONNECTION, ))
        PROCESSES.start()
        PROCESSES.join()
        OUTPUT = PARENT_CONNECTION.recv()
    ...
    def test_scan_some_open_ports(self):
